...
//                        """
                                sh """
                                    ...
                                    cat last_tag | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["data"]["repository"]["refs"]["nodes"][0]["name"]'
                                """
                            }

                            def LAST_TAG = sh(
                                    script: "cat last_tag | python -c 'import json,sys;obj=json.load(sys.stdin);print obj[\"data\"][\"repository\"][\"refs\"][\"nodes\"][0][\"name\"]",
                                    returnStdout: true
                            ).trim()
